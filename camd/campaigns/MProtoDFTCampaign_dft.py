...
def load_candidate(m3gnet=False):
    if m3gnet:
        data = pd.read_pickle("data/candidate_m3gnet_data_wf.pickle")
    else:
        data = pd.read_pickle("data/candidate_data_wf.pickle")
    return data

def load_seed(chemsys=None):
    """
    Helper function
    Returns:
        ...
        in the chemsys
    """
    if not os.path.isfile("data/mp_binary.pickle"):
        with MPRester() as mpr:
            #entries = mpr.get_entries_in_chemsys(chemsys, inc_structure=True)
            entries = mpr.get_entries({"nsites":{"$lte":2}}, inc_structure=True)
            data = pd.DataFrame(entries)
            data.to_pickle("data/mp_binary.pickle")
    else:
        data = pd.read_pickle("data/mp_binary.pickle")
...
if __name__ == "__main__":
    ...
    DB_FILE = "/home/ubuntu/repos/cz_calc/atomate/config/db.json"
    LPAD_FILE = "/home/ubuntu/repos/cz_calc/atomate/config/my_launchpad.yaml"
    # Parameter
    CHEMSYS = "Si"
